#!/usr/bin/env python3
"""
Fetch hourly London wind speed and direction from Kew Gardens (MIDAS Open).
...
Kew Gardens (src_id 723) is the nearest still-active MIDAS mean-wind station to
central London after Battersea Heliport (~8 km from Battersea, ~11 km from
London Marylebone Road). Exports native hourly mean wind speed (knots) and
direction (degrees true). Resampling and daily alignment are done in the notebook.

Usage:
  export CEDA_USERNAME='your_ceda_username'
  export CEDA_PASSWORD='your_ceda_password'
  python features/scripts/fetch_london_wind_kew_gardens.py

  python features/scripts/fetch_london_wind_kew_gardens.py --start-year 2022 --end-date 2025-12-31

  # Optional: filter Kew Gardens rows from an existing hourly CSV:
  python features/scripts/fetch_london_wind_kew_gardens.py --from-local path/to/hourly_wind.csv
...
DEFAULT_OUTPUT = REPO_ROOT / "data/london_wind_kew_gardens_hourly.csv"
...
def _collect_kew_download_urls(
    dataset_version: str,
    start_year: int,
    end_year: int,
) -> list[str]:
...
def _fetch_kew_hourly(
    ...
    urls = _collect_kew_download_urls(dataset_version, start_year, clipped_end.year)
    if not urls:
        raise RuntimeError("No Kew Gardens MIDAS wind files found for the requested period.")
...
def fetch_kew_gardens_wind_timeseries(
    ...
    """Fetch hourly Kew Gardens wind observations from MIDAS Open."""
    ...
    if from_local:
        ...
        hourly, source_label = _fetch_kew_hourly(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download hourly Kew Gardens wind speed and direction from MIDAS Open."
    )
...
def main() -> int:
    ...
    frame = fetch_kew_gardens_wind_timeseries(
