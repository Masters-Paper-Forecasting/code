#!/usr/bin/env python3
"""
Fetch hourly precipitation from Northolt (MIDAS Open).
...
Northolt (src_id 709, ~19 km) is the nearest Greater London MIDAS Open hourly
raingauge after St James's Park and Kew Gardens for remaining precipitation gaps
(including late-2025 coverage in dataset-version-202607). Same station already used
for pressure/humidity backfill. Exports hourly precipitation amount (prcp_amt, mm)
and duration (prcp_dur, minutes). Only rows with ob_hour_count == 1 are kept as
...
Usage:
  ...
  python features/scripts/fetch_london_precipitation_northolt.py

  python features/scripts/fetch_london_precipitation_northolt.py --start-year 2022 --end-date 2025-12-31

  # Optional: filter Northolt rows from an existing hourly CSV:
  python features/scripts/fetch_london_precipitation_northolt.py --from-local path/to/hourly_rain.csv
...
DEFAULT_OUTPUT = REPO_ROOT / "data/london_precipitation_northolt_hourly.csv"
...
def fetch_london_precipitation_northolt_timeseries(
    ...
    """Fetch hourly Northolt precipitation observations from MIDAS Open."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download hourly Northolt precipitation from MIDAS Open."
    )
...
def main() -> int:
    ...
    frame = fetch_london_precipitation_northolt_timeseries(
