#!/usr/bin/env python3
"""
Fetch hourly precipitation from London St James's Park (MIDAS Open).
...
London St James's Park (src_id 697) is the closest Greater London MIDAS Open
hourly raingauge to London Marylebone Road (~2.6 km) and is the primary
precipitation station for this project. Battersea Heliport (wind and pressure)
does not appear in the hourly rainfall collection. Exports hourly
precipitation amount (prcp_amt, mm) and duration (prcp_dur, minutes). Only rows
with ob_hour_count == 1 are kept as true hourly totals. Resampling and daily
alignment are done in the notebook.

Usage:
  export CEDA_USERNAME='your_ceda_username'
  export CEDA_PASSWORD='your_ceda_password'
  python features/scripts/fetch_london_precipitation_st_james_park.py

  python features/scripts/fetch_london_precipitation_st_james_park.py --start-year 2022 --end-date 2025-12-31

  # Optional: filter St James's Park rows from an existing hourly CSV:
  python features/scripts/fetch_london_precipitation_st_james_park.py --from-local path/to/hourly_rain.csv
...
DEFAULT_OUTPUT = REPO_ROOT / "data/london_precipitation_st_james_park_hourly.csv"
...
def _fetch_st_james_hourly(
    ...
    if not urls:
        raise RuntimeError(
            "No St James's Park MIDAS hourly rainfall files found for the requested period."
        )
...
def fetch_london_precipitation_timeseries(
    ...
    """Fetch hourly St James's Park precipitation observations from MIDAS Open."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download hourly St James's Park precipitation from MIDAS Open."
    )
...
def main() -> int:
    ...
    frame = fetch_london_precipitation_timeseries(
