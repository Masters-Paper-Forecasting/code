#!/usr/bin/env python3
"""
Fetch hourly air pressure from Northolt (MIDAS Open).
...
Only four Greater London stations include pressure in MIDAS Open hourly weather
files. Battersea Heliport (src_id 62084, ~6 km) is closest to London Marylebone Road
and is fetched by features/scripts/fetch_london_pressure_battersea.py. Northolt (src_id 709, ~19 km)
is the nearest alternative with pressure data and can backfill Battersea gaps
(e.g. Apr-May 2025). Exports native hourly msl_pressure (hPa) and stn_pres (hPa).
Resampling and daily alignment are done in the notebook.

Usage:
  export CEDA_USERNAME='your_ceda_username'
  export CEDA_PASSWORD='your_ceda_password'
  python features/scripts/fetch_london_pressure_northolt.py

  python features/scripts/fetch_london_pressure_northolt.py --start-year 2022 --end-date 2025-12-31

  # Optional: filter Northolt rows from an existing hourly CSV:
  python features/scripts/fetch_london_pressure_northolt.py --from-local path/to/hourly_weather.csv
...
STATION_FILE_NAME = "northolt"
DEFAULT_OUTPUT = REPO_ROOT / "data/london_pressure_northolt_hourly.csv"

PRESSURE_COLUMNS = ["ob_time", "src_id", "msl_pressure", "stn_pres"]
...
def _parse_pressure_file(payload: bytes, start: pd.Timestamp, end: pd.Timestamp) -> pd.DataFrame:
    rows = _parse_badc_csv(payload.decode("utf-8"))
    if not rows:
        return pd.DataFrame(columns=PRESSURE_COLUMNS)

    frame = pd.DataFrame(rows)
    if "version_num" in frame.columns:
        frame = frame[frame["version_num"].astype(str) == "1"]

    keep = [col for col in PRESSURE_COLUMNS if col in frame.columns]
    ...
    frame["msl_pressure"] = _to_numeric(frame["msl_pressure"])
    if "stn_pres" in frame.columns:
        frame["stn_pres"] = _to_numeric(frame["stn_pres"])
...
def _fetch_northolt_hourly(
    ...
    if not urls:
        raise RuntimeError(
            "No Northolt MIDAS hourly weather files found for the requested period."
        )

    frames: list[pd.DataFrame] = []
    for url in urls:
        frame = _parse_pressure_file(download(url), start, end_ts)
        if not frame.empty:
            frames.append(frame)
...
def fetch_london_pressure_timeseries(
    ...
    """Fetch hourly Northolt pressure observations from MIDAS Open."""
    ...
    if from_local:
        ...
        with from_local.open(newline="") as handle:
            for row in csv.DictReader(handle):
                ...
                rows.append(
                    {
                        "ob_time": row["ob_time"],
                        "msl_pressure": row.get("msl_pressure"),
                        "stn_pres": row.get("stn_pres"),
                    }
                )

        if not rows:
            raise RuntimeError(
                f"No Northolt hourly rows found in {from_local} "
        ...
        hourly["msl_pressure"] = _to_numeric(hourly["msl_pressure"])
        hourly["stn_pres"] = _to_numeric(hourly["stn_pres"])
        hourly = hourly.drop_duplicates(subset=["ob_time"], keep="last")
        hourly = hourly.sort_values("ob_time").reset_index(drop=True)
    else:
        hourly, source_label = _fetch_northolt_hourly(
    ...
    if hourly["msl_pressure"].notna().any():
        print(
            f"  msl_pressure range: {hourly['msl_pressure'].min():.1f} to "
            f"{hourly['msl_pressure'].max():.1f} hPa"
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download hourly Northolt air pressure from MIDAS Open."
    )
...
def main() -> int:
    ...
    frame = fetch_london_pressure_timeseries(
