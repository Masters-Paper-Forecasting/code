#!/usr/bin/env python3
"""
Fetch hourly air pressure from Battersea Heliport (MIDAS Open).
...
Central London stations such as St James's Park do not include pressure values in
MIDAS Open hourly weather files. Battersea Heliport (src_id 62084) is the closest
Greater London station to London Marylebone Road (~6 km) with hourly mean sea level
pressure (msl_pressure, hPa) and station pressure (stn_pres, hPa). Resampling and
daily alignment are done in the notebook.

Usage:
  export CEDA_USERNAME='your_ceda_username'
  export CEDA_PASSWORD='your_ceda_password'
  python features/scripts/fetch_london_pressure_battersea.py

  python features/scripts/fetch_london_pressure_battersea.py --start-year 2022 --end-date 2025-12-31

  # Optional: filter Battersea rows from an existing hourly CSV:
  python features/scripts/fetch_london_pressure_battersea.py --from-local path/to/hourly_weather.csv
...
DEFAULT_OUTPUT = REPO_ROOT / "data/london_pressure_hourly.csv"
...
def _fetch_battersea_hourly(
    ...
    if not urls:
        raise RuntimeError(
            "No Battersea MIDAS hourly weather files found for the requested period."
        )
...
def fetch_london_pressure_timeseries(
    ...
    """Fetch hourly Battersea pressure observations from MIDAS Open."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download hourly Battersea air pressure from MIDAS Open."
    )
