#!/usr/bin/env python3
"""
Fetch hourly relative humidity from Northolt (MIDAS Open).
...
Northolt (src_id 709, ~19 km) is the nearest Greater London MIDAS hourly weather
alternative after Battersea Heliport and St James's Park, and can backfill remaining
humidity gaps (e.g. Apr-May 2025). Same pattern as pressure backfill.
Exports calculated relative humidity (rltv_hum, %) and dewpoint (dewpoint, deg C).
...
Usage:
  ...
  python features/scripts/fetch_london_humidity_northolt.py

  python features/scripts/fetch_london_humidity_northolt.py --start-year 2022 --end-date 2025-12-31

  # Optional: filter Northolt rows from an existing hourly CSV:
  python features/scripts/fetch_london_humidity_northolt.py --from-local path/to/hourly_weather.csv
...
DEFAULT_OUTPUT = REPO_ROOT / "data/london_humidity_northolt_hourly.csv"
...
def fetch_london_humidity_northolt_timeseries(
    ...
    """Fetch hourly Northolt humidity observations from MIDAS Open."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download hourly Northolt relative humidity from MIDAS Open."
    )
...
def main() -> int:
    ...
    frame = fetch_london_humidity_northolt_timeseries(
