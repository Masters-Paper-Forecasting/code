#!/usr/bin/env python3
"""
Fetch hourly relative humidity from Battersea Heliport (MIDAS Open).
...
Battersea Heliport (src_id 62084) is the closest Greater London hourly weather
station to London Marylebone Road (~6 km). Exports calculated relative humidity
(rltv_hum, %) and dewpoint (dewpoint, deg C). Resampling and daily alignment are
done in the notebook.

Usage:
  export CEDA_USERNAME='your_ceda_username'
  export CEDA_PASSWORD='your_ceda_password'
  python features/scripts/fetch_london_humidity_battersea.py

  python features/scripts/fetch_london_humidity_battersea.py --start-year 2022 --end-date 2025-12-31

  # Optional: filter Battersea rows from an existing hourly CSV:
  python features/scripts/fetch_london_humidity_battersea.py --from-local path/to/hourly_weather.csv
...
DEFAULT_OUTPUT = REPO_ROOT / "data/london_humidity_hourly.csv"
...
def fetch_london_humidity_timeseries(
    ...
    """Fetch hourly Battersea humidity observations from MIDAS Open."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download hourly Battersea relative humidity from MIDAS Open."
    )
...
def main() -> int:
    ...
    frame = fetch_london_humidity_timeseries(
