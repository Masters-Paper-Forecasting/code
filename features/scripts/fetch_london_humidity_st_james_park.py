#!/usr/bin/env python3
"""
Fetch hourly relative humidity from London St James's Park (MIDAS Open).

Source: Met Office MIDAS Open UK hourly weather observations (CEDA).
https://catalogue.ceda.ac.uk/uuid/99173f6a802147aeba430d96d2bb3099/

London St James's Park (src_id 697) is the closest Greater London hourly weather
station to London Marylebone Road (~2.6 km) and can backfill Battersea humidity
gaps. Exports calculated relative humidity (rltv_hum, %) and dewpoint
(dewpoint, deg C). Resampling and daily alignment are done in the notebook.

Usage:
  export CEDA_USERNAME='your_ceda_username'
  export CEDA_PASSWORD='your_ceda_password'
  python features/scripts/fetch_london_humidity_st_james_park.py

  python features/scripts/fetch_london_humidity_st_james_park.py --start-year 2022 --end-date 2025-12-31

  # Optional: filter St James's Park rows from an existing hourly CSV:
  python features/scripts/fetch_london_humidity_st_james_park.py --from-local path/to/hourly_weather.csv
...
MIDAS_HOURLY_WEATHER_ROOT = "/badc/ukmo-midas-open/data/uk-hourly-weather-obs"
MIDAS_QC_VERSION = "qc-version-1"
CEDA_END_DATE = date(2025, 12, 31)

STATION_NAME = "LONDON ST JAMES'S PARK"
SRC_ID = "697"
HISTORIC_COUNTY = "greater-london"
STATION_FILE_NAME = "london-st-jamess-park"
DEFAULT_OUTPUT = REPO_ROOT / "data/london_humidity_st_james_park_hourly.csv"

HUMIDITY_COLUMNS = ["ob_time", "src_id", "rltv_hum", "dewpoint"]
...
def _latest_dataset_version() -> str:
    listing = _fetch_json(f"{CEDA_ARCHIVE_BROWSER}{MIDAS_HOURLY_WEATHER_ROOT}/?json")
    versions = sorted(
        item["name"]
    ...
    if not versions:
        raise RuntimeError(
            f"No MIDAS hourly weather dataset versions found under {MIDAS_HOURLY_WEATHER_ROOT}"
        )
    return versions[-1]
...
def _parse_humidity_file(payload: bytes, start: pd.Timestamp, end: pd.Timestamp) -> pd.DataFrame:
    rows = _parse_badc_csv(payload.decode("utf-8"))
    if not rows:
        return pd.DataFrame(columns=HUMIDITY_COLUMNS)

    frame = pd.DataFrame(rows)
    if "version_num" in frame.columns:
        frame = frame[frame["version_num"].astype(str) == "1"]

    keep = [col for col in HUMIDITY_COLUMNS if col in frame.columns]
    frame = frame[keep].copy()
    frame["ob_time"] = pd.to_datetime(frame["ob_time"], errors="coerce")
    frame = frame.dropna(subset=["ob_time"])
    frame = frame[(frame["ob_time"] >= start) & (frame["ob_time"] <= end)]
    frame["rltv_hum"] = _to_numeric(frame["rltv_hum"])
    if "dewpoint" in frame.columns:
        frame["dewpoint"] = _to_numeric(frame["dewpoint"])
    frame["src_id"] = frame["src_id"].astype(str).str.lstrip("0").replace("", "0")
    frame = frame.drop_duplicates(subset=["ob_time"], keep="last")
    return frame
...
def _date_bounds(start_year: int, end_date: date) -> tuple[pd.Timestamp, pd.Timestamp, date]:
    ...
    if start.date() > clipped_end:
        raise RuntimeError(
            ...
            f"MIDAS Open UK hourly weather currently runs through {CEDA_END_DATE}."
        )
...
def _collect_station_download_urls(
    ...
    county_listing = _fetch_json(
        f"{CEDA_ARCHIVE_BROWSER}{MIDAS_HOURLY_WEATHER_ROOT}/{dataset_version}/?json"
    )
    county_path = next(
        item["path"]
...
def _fetch_st_james_hourly(
    ...
    if not urls:
        raise RuntimeError(
            "No St James's Park MIDAS hourly weather files found for the requested period."
        )

    frames: list[pd.DataFrame] = []
    for url in urls:
        frame = _parse_humidity_file(download(url), start, end_ts)
        if not frame.empty:
            frames.append(frame)
    ...
    hourly = pd.concat(frames, ignore_index=True).sort_values("ob_time").reset_index(drop=True)
    hourly = hourly.drop_duplicates(subset=["ob_time"], keep="last")
    return hourly, source_label


def fetch_london_humidity_st_james_timeseries(
    ...
    """Fetch hourly St James's Park humidity observations from MIDAS Open."""
    ...
    if from_local:
        ...
        if not rows:
            raise RuntimeError(
                f"No St James's Park hourly rows found in {from_local} "
                f"for {start_text} to {end_text}."
            )

        hourly = pd.DataFrame(rows)
        hourly["ob_time"] = pd.to_datetime(hourly["ob_time"], errors="coerce")
        hourly = hourly.dropna(subset=["ob_time"])
        hourly = hourly[(hourly["ob_time"] >= start) & (hourly["ob_time"] <= end_ts)]
        hourly["rltv_hum"] = _to_numeric(hourly["rltv_hum"])
        hourly["dewpoint"] = _to_numeric(hourly["dewpoint"])
        hourly = hourly.drop_duplicates(subset=["ob_time"], keep="last")
        hourly = hourly.sort_values("ob_time").reset_index(drop=True)
    else:
        hourly, source_label = _fetch_st_james_hourly(
    ...
    print(
        ...
        f"({hourly['ob_time'].min()} to {hourly['ob_time'].max()})."
    )
    if hourly["rltv_hum"].notna().any():
        print(
            f"  rltv_hum range: {hourly['rltv_hum'].min():.1f} to "
            f"{hourly['rltv_hum'].max():.1f} %"
        )
    if hourly["dewpoint"].notna().any():
        print(
            f"  dewpoint range: {hourly['dewpoint'].min():.1f} to "
            f"{hourly['dewpoint'].max():.1f} deg C"
        )
    if end > CEDA_END_DATE:
        print(
            f"  Note: MIDAS Open UK hourly weather data currently runs through {CEDA_END_DATE}. "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download hourly St James's Park relative humidity from MIDAS Open."
    )
...
def main() -> int:
    ...
    frame = fetch_london_humidity_st_james_timeseries(
