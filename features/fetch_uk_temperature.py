...
Daily admin-region temperature (default):
  ...
  - Historical data (2016 through 2024): CEDA HadUK-Grid admin-region files.
  - 2025+ provisional grid data is not split by region and is skipped.
...
Usage:
  ...
  python features/fetch_uk_temperature.py

  # Or local NetCDF files downloaded from CEDA:
  python features/fetch_uk_temperature.py \\
    --ceda-tasmax-file ~/Downloads/tasmax_hadukgrid_uk_region_day_19310101-20241231.nc \\
    --ceda-tasmin-file ~/Downloads/tasmin_hadukgrid_uk_region_day_19310101-20241231.nc
...
CEDA_HADUK_VERSION = "v1.3.1.ceda"
CEDA_HADUK_RELEASE = "v20250415"
FILL_VALUE = 1e20
...
def fetch_daily_temperature_by_region(
    ...
    if start > end:
        raise RuntimeError(
            ...
            "HadUK-Grid admin-region daily files currently run through 2024."
        )
    ...
    if (end_date or date.today()) > CEDA_END_DATE:
        print(
            "  Note: 2025+ provisional Met Office data is gridded only, "
            "so regional daily values are not included."
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("data/uk_temperature_by_region.csv"),
